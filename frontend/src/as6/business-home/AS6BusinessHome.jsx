...
export const AS6_BUSINESS_HOME_VERSION = "EPIC004_PR5";
...
export function AS6BusinessHome() {
  ...
  const [executiveAutomationQueue, setExecutiveAutomationQueue] = useState([]);
  const [executiveAutomationCurrentScenario, setExecutiveAutomationCurrentScenario] = useState(null);
  const [executiveAutomationRecovery, setExecutiveAutomationRecovery] = useState({ status: "idle", retryCount: 0, lastFailureReason: null, lastSuccessfulStep: 0, recoveringFromStep: null });
  ...
  function enqueueAS6ExecutiveAutomationScenario(scenarioId) {
    const scenario = AS6_EXECUTIVE_AUTOMATION_SCENARIOS[scenarioId];
    if (!scenario) return;
    setExecutiveAutomationQueue((queue) => [...queue, { id: "queue-" + Date.now(), scenarioId, title: scenario.title, status: "queued" }]);
  }

  function removeAS6ExecutiveAutomationScenarioFromQueue(queueId) {
    setExecutiveAutomationQueue((queue) => queue.filter((item) => item.id !== queueId));
  }

  function clearAS6ExecutiveAutomationQueue() {
    setExecutiveAutomationQueue([]);
    setExecutiveAutomationCurrentScenario(null);
  }

  function retryAS6ExecutiveAutomationScenario() {
    const scenarioId = executiveAutomationCurrentScenario?.scenarioId || executiveAutomationPlan?.scenarioId;
    if (!scenarioId) return;
    setExecutiveAutomationRecovery((state) => ({ ...state, status: "retrying", retryCount: state.retryCount + 1, lastFailureReason: executiveAutomationPipeline?.reason || null }));
    handleAS6ExecutiveAutomationScenario(scenarioId);
    setExecutiveAutomationRecovery((state) => ({ ...state, status: "idle", lastSuccessfulStep: executiveAutomationPipeline?.currentStep || 0 }));
  }

  function recoverAS6ExecutiveAutomationScenario() {
    const scenarioId = executiveAutomationCurrentScenario?.scenarioId || executiveAutomationPlan?.scenarioId;
    if (!scenarioId) return;
    const lastStep = executiveAutomationPipeline?.currentStep || executiveAutomationRecovery.lastSuccessfulStep || 0;
    setExecutiveAutomationRecovery((state) => ({ ...state, status: "recovering", retryCount: state.retryCount + 1, recoveringFromStep: lastStep, lastFailureReason: executiveAutomationPipeline?.reason || "Recovery requested" }));
    handleAS6ExecutiveAutomationScenario(scenarioId);
    setExecutiveAutomationRecovery((state) => ({ ...state, status: "idle", lastSuccessfulStep: Math.max(lastStep, executiveAutomationPipeline?.currentStep || 0) }));
  }

  function retryAS6ExecutiveAutomationStep() {
    const scenarioId = executiveAutomationCurrentScenario?.scenarioId || executiveAutomationPlan?.scenarioId;
    if (!scenarioId) return;
    setExecutiveAutomationRecovery((state) => ({ ...state, status: "retrying", retryCount: state.retryCount + 1, recoveringFromStep: executiveAutomationPipeline?.currentStep || 0, lastFailureReason: executiveAutomationPipeline?.reason || "Step retry requested" }));
    handleAS6ExecutiveAutomationScenario(scenarioId);
    setExecutiveAutomationRecovery((state) => ({ ...state, status: "idle" }));
  }

  function runAS6ExecutiveAutomationQueue() {
    setExecutiveAutomationQueue((queue) => {
      const [nextScenario, ...rest] = queue;
      if (!nextScenario) return queue;
      setExecutiveAutomationCurrentScenario({ ...nextScenario, status: "running" });
      handleAS6ExecutiveAutomationScenario(nextScenario.scenarioId);
      return rest;
    });
  }
