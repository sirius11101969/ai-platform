...
const kpiBlueprint = [
  ...
  { key: "aiEmployees", label: "AS6 Сотрудники", value: demoMetrics.aiEmployees, delta: "Активны", tone: "ai", icon: <img src={as6Robot} alt="AS6 AI Workforce" className="as6-ai-kpi-robot-image" />, spark: [52, 50, 54, 53, 55, 56, 58] },
  { key: 'conversion', label: 'Конверсия', value: demoMetrics.conversion, delta: '+8.3%', tone: 'conversion', icon: '◎', spark: [21, 26, 24, 31, 35, 39, 46] },
  { key: 'revenueRisk', label: 'Revenue at Risk', value: '487 000 ₽', delta: 'контроль', tone: 'risk', icon: '!', spark: [46, 42, 38, 40, 35, 31, 28] },
]
...
const executiveModules = [
  ...
  { title: 'AI Workforce', text: 'AS6 Сотрудники, загрузка, эффективность и задачи', href: '/ai-workforce-center', icon: <img src={as6Robot} alt="AI Workforce" className="executive-module-robot-icon" />, tone: 'violet' },
...
const quickActionLinks = {
  ...
  "Запустить follow-up": "/ai-approval-center",
  "Запустить scoring": "/dashboard/revenue",
  "Открыть Revenue Brain": "/ai-executive-brain",
  "Настроить AS6": "/ai-enterprise-coordination",
}

const quickActions = ['Создать лид', 'Создать сделку', 'Добавить задачу', 'Запустить follow-up', 'Запустить scoring', 'Открыть Revenue Brain', 'Настроить AS6']
const chartBars = [46, 64, 56, 78, 70, 92, 82, 100]
const as6RevenueFlowStages = [
  { title: 'Новый', deals: [{ company: 'Astra Retail', amount: '+187 000 ₽', score: '91', priority: 'High', probability: '72%', date: 'сегодня' }] },
  { title: 'Квалификация', deals: [{ company: 'Nord Cloud', amount: '+96 000 ₽', score: '84', priority: 'Medium', probability: '64%', date: 'завтра' }] },
  { title: 'Предложение', deals: [{ company: 'Vector Logistics', amount: '+128 000 ₽', score: '88', priority: 'High', probability: '78%', date: '29 июн' }] },
  { title: 'Встреча', deals: [{ company: 'Prime Finance', amount: '+52 000 ₽', score: '79', priority: 'Watch', probability: '58%', date: '30 июн' }] },
  { title: 'Успешно', deals: [{ company: 'Orbit SaaS', amount: '+24 000 ₽', score: '96', priority: 'Won', probability: '100%', date: 'сегодня' }] },
]
...
const [apiState, setApiState] = useState({})
  ...
  const actionQueue = useMemo(() => {
    ...
    return [
      { title: 'Одобрить AI follow-up', meta: 'Запустить follow-up · высокий приоритет', value: '+$2,800' },
  ...
  return (
    <>
    <main className="command-center-page" data-command-center-visual="premium-as6" data-as6-diagnostic-page="as6-one-pixel-perfect-command-center" data-production-markers="AS6OnePage as6-one as6-command-center-visual-foundation as6-one-exact-command-center-clone as6-one-pixel-perfect-command-center as6-revenue-flow as6-general-director as6-core"><section className="command-hero" data-as6-diagnostic-hero="executive-command-center">
        <div>
          <h1>AS6 ONE <span>Командный центр</span></h1>
          <p>Первая цифровая форма жизни для бизнеса</p>
      ...
      <section className="command-recommendations" data-ai-recommendations data-as6-one-recommendations="first">
        <h2>Сегодня AS6 рекомендует</h2><div>{[
          { title: 'Закрыть 4 сделки', text: 'Ответить 12 клиентам и назначить встречу.', action: 'Выполнить план AS6', href: '/crm?filter=as6-plan', tone: 'violet', icon: '✓' },
          { title: 'Перезапустить 3 зависшие сделки', text: 'Прогноз роста выручки +487 000 ₽.', action: 'Открыть план', href: '/ai-executive-brain', tone: 'cyan', icon: '↗' },
        ].map((card) => <a className={`recommendation ${card.tone}`} href={card.href} key={card.title} aria-label={card.action}><b>{card.icon}</b><strong>{card.title}</strong><p>{card.text}</p><span className="recommendation-action">{card.action} →</span></a>)}</div>
      </section>
      ...
      <section className="command-main-grid">
        <div className="command-core">
          <section className="command-top-grid">
            <article className="command-card daily-goal">
              <div className="command-card-head"><h2>Revenue Brain</h2><span>68%</span></div>
            ...
            <article className="command-card pipeline-card" data-as6-revenue-flow="columns">
              ...
              <div className="pipeline-layout as6-revenue-flow">
                {as6RevenueFlowStages.map((stage) => <div className="pipeline-summary" key={stage.title}><small>{stage.title}</small>{stage.deals.map((deal) => <div className="next-action" key={deal.company}><span>{deal.company}<small>{deal.priority} · AS6 score {deal.score} · {deal.probability} · {deal.date}</small></span><strong>{deal.amount}</strong></div>)}</div>)}
              </div>
            </article>
          </section>
        ...
        <aside className="command-right-rail" data-right-action-rail data-copilot-rail data-as6-diagnostic-copilot="right-action-rail">
          ...
          <article className="command-card next-action-card"><h2>Рекомендация AS6</h2>{nextActions.map((action) => <div className="next-action" key={action.label}><b>{action.icon}</b><span>{action.label}<small>{action.note}</small></span><strong>{action.value}</strong></div>)}<div className="expected-effect"><span>Ожидаемый эффект</span><strong>+$8,900</strong></div></article>
        <article className="command-card event-card"><div className="command-card-head"><h2>Состояние системы</h2><a href="/ai-system-health-center">AS6 Core</a></div>{healthSummary.map((item) => <div className="event-row" key={item.label}><b>●</b><span>{item.label}</span><time>{item.value}</time></div>)}</article>
        </aside>
      </section>

      <section className="quick-actions quick-actions-primary" data-as6-bottom-status="true"><h2>AS6 Core</h2><div>{['AS6 работает', 'CRM синхронизирована', 'Почта подключена', 'Telegram подключён', 'WhatsApp подключён', 'Все сервисы онлайн', `Обновлено: ${updatedAt}`].map((status) => <a href="/ai-system-health-center" role="button" key={status} aria-label={status}><span>●</span>{status}</a>)}</div></section>

      <section className="command-recommendations" data-ai-recommendations>
        <h2>Сегодня AS6 рекомендует</h2><div>{recommendations.map((card) => <a className={`recommendation ${card.tone}`} href={card.href} key={card.title} aria-label={card.action}><b>{card.icon}</b><strong>{card.title}</strong><p>{card.text}</p><span className="recommendation-action">{card.action} →</span></a>)}</div>
      </section>
    </main>
    </>
  )
