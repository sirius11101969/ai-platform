...
const kpiBlueprint = [
  ...
  { key: "aiEmployees", label: "AS6 Сотрудники", value: demoMetrics.aiEmployees, delta: "Активны", tone: "ai", icon: <img src={as6Robot} alt="AS6 AS6 Сотрудники" className="as6-ai-kpi-robot-image" />, spark: [52, 50, 54, 53, 55, 56, 58] },
...
const aiEmployees = [
  { name: 'AS6 SDR Agent', value: 156, suffix: '', progress: 86, delta: '+24%', avatar: <img src={as6Robot} alt="AS6 AS6 SDR Agent" className="agent-avatar-image" />, tone: 'cyan' },
  { name: 'AS6 Closer Agent', value: '$842k', suffix: '', progress: 78, delta: '+31%', avatar: <img src={as6Robot} alt="AS6 AS6 Closer Agent" className="agent-avatar-image" />, tone: 'violet' },
  { name: 'AS6 Support Agent', value: 98, suffix: '%', progress: 72, delta: '+8%', avatar: <img src={as6Robot} alt="AS6 AS6 Support Agent" className="agent-avatar-image" />, tone: 'orange' },
  { name: 'AS6 Analyst', value: 124, suffix: '', progress: 91, delta: '+15%', avatar: <img src={as6Robot} alt="AS6 AS6 Analyst" className="agent-avatar-image" />, tone: 'green' },
]
...
const executiveModules = [
  ...
  { title: 'AS6 Сотрудники', text: 'AS6 Сотрудники, загрузка, эффективность и задачи', href: '/ai-workforce-center', icon: <img src={as6Robot} alt="AS6 Сотрудники" className="executive-module-robot-icon" />, tone: 'violet' },
...
const [apiState, setApiState] = useState({})
  ...
  const executiveBrief = useMemo(() => {
    ...
    return {
      ...
      opportunity: brief.opportunity || brief.opportunities?.[0] || 'Ускорить обработку лидов через AS6 Сотрудники и follow-up automation.',
    }
  }, [apiState])
  ...
  const systemStatus = useMemo(() => ([
    ...
    { label: 'AS6 Сотрудники', value: pickMetric(apiState.kpi?.kpis?.workforceStatus, 'READY') },
  ...
  const businessSummary = useMemo(() => ([
    ...
    { label: 'AS6 Сотрудники', value: pickMetric(apiState.kpi?.kpis?.workforceUtilization, 'READY'), note: 'Исполнение и автоматизация' },
  ...
  const executiveStream = useMemo(() => {
    const items = [
      ...
      { type: 'Approvals', title: 'Очередь решений', text: `${actionQueue.length} AS6-действия готовы к проверке.`, status: actionQueue.length ? 'ACTION' : 'CLEAR' },
  ...
  return (
    ...
    <main className="command-center-page" data-command-center-visual="premium-as6" data-as6-diagnostic-page="as6-one-command-center-clone" data-as6-one-production-markers="AS6OnePage as6-one as6-command-center-visual-foundation as6-one-exact-command-center-clone as6-revenue-flow as6-general-director as6-core"><section className="command-hero" data-as6-diagnostic-hero="executive-command-center">
      ...
      <section className="quick-actions quick-actions-primary" data-quick-actions data-as6-primary-actions="above-fold"><h2>AS6 задачи</h2><div>{quickActions.map((action, index) => <a href={quickActionLinks[action] || "#"} role="button" key={action} aria-label={action}><span>{["👥", "$", "🧾", "🛡️", "〽️", "📊", <img src={as6Robot} alt="AI" className="quick-action-robot-icon" />][index]}</span>{action}</a>)}</div></section>
        ...
        <div className="command-core">
          <section className="command-top-grid">
            <article className="command-card daily-goal">
              ...
              <div className="goal-stats">
                <div><small>План AS6</small><strong>4 сделки</strong></div>
                <div><small>Клиенты</small><strong>12 ответов</strong></div>
                <div><small>Прогноз роста</small><strong>+487 000 ₽</strong></div>
              </div><a className="copilot-action-link" href="/ai-execution-center">Выполнить план AS6</a>
            </article>
            ...
            <article className="command-card pipeline-card" data-pipeline-funnel>
              ...
              <div className="pipeline-layout">
                ...
                <div className="pipeline-summary"><small>Прогноз</small><strong>{pipelineSummary.process}</strong><small>Горячие лиды</small><strong>{pipelineSummary.won}</strong><em>AS6-оценка {pipelineSummary.growth}</em><small>Риск</small><strong>Здоровье воронки: 86%</strong><a className="copilot-action-link" href="/ai-revenue-intelligence">Открыть Revenue Brain</a></div>
              </div>
            </article>
          </section>
          ...
          <section className="command-second-grid">
            ...
            <article className="command-card revenue-dynamics">
              ...
              <div className="goal-stats"><div><small>Прогноз</small><strong>+487 000 ₽</strong></div><div><small>Риск</small><strong>Средний</strong></div><div><small>Здоровье воронки</small><strong>86%</strong></div></div><strong className="chart-total">{revenueRange === '1d' ? '$428,000' : revenueRange === '7d' ? '$2,940,000' : revenueRange === '90d' ? '$31,800,000' : revenueRange === '365d' ? '$128,400,000' : '$12,490,000'} <em>{revenueRange === '1d' ? '+3.1%' : revenueRange === '7d' ? '+9.8%' : '+22.4%'}</em></strong>
              <div className="mock-chart">{chartBars.map((height, index) => <span key={index} style={{ height: `${height}%` }} />)}<svg viewBox="0 0 240 100" preserveAspectRatio="none"><polyline points="0,76 34,58 68,66 102,38 136,52 170,24 204,36 240,12" /></svg></div><a className="copilot-action-link" href="/ai-revenue-intelligence">Открыть Revenue Brain</a>
            </article>
        ...
        <aside className="command-right-rail" data-right-action-rail data-copilot-rail data-as6-diagnostic-copilot="right-action-rail">
          <article className="command-card copilot-hero" data-copilot-hero>
            ...
            <div className="copilot-inline-layout">
              ...
              <a className="copilot-action-link" href="/ai-executive-brain">Спросить AS6 Помощник</a>
            </div>
          </article>
          ...
          <article className="command-card next-action-card"><h2>Лучшее следующее действие</h2>{nextActions.map((action) => <div className="next-action" key={action.label}><b>{action.icon}</b><span>{action.label}<small>{action.note}</small></span><strong>{action.value}</strong></div>)}<div className="expected-effect"><span>AS6-оценка</span><strong>+$8,900</strong></div></article>
        </aside>
      </section>
