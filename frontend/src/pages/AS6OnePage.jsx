...
const kpiBlueprint = [
  ...
  { key: "aiEmployees", label: "AS6 Сотрудники", value: demoMetrics.aiEmployees, delta: "Активны", tone: "ai", icon: <img src={as6Robot} alt="AS6 Workforce" className="as6-ai-kpi-robot-image" />, spark: [52, 50, 54, 53, 55, 56, 58] },
...
const aiEmployees = [
  { name: 'AS6 SDR Agent', value: 156, suffix: '', progress: 86, delta: '+24%', avatar: <img src={as6Robot} alt="AS6 SDR Agent" className="agent-avatar-image" />, tone: 'cyan' },
  { name: 'AS6 Closer Agent', value: '$842k', suffix: '', progress: 78, delta: '+31%', avatar: <img src={as6Robot} alt="AS6 Closer Agent" className="agent-avatar-image" />, tone: 'violet' },
  { name: 'AS6 Support Agent', value: 98, suffix: '%', progress: 72, delta: '+8%', avatar: <img src={as6Robot} alt="AS6 Support Agent" className="agent-avatar-image" />, tone: 'orange' },
  { name: 'AS6 Analyst', value: 124, suffix: '', progress: 91, delta: '+15%', avatar: <img src={as6Robot} alt="AS6 Analyst" className="agent-avatar-image" />, tone: 'green' },
]

const events = [
  { icon: '✉️', text: 'AS6 SDR Agent отправил письмо лиду', time: '2 мин назад' },
...
const nextActions = [
  ...
  { label: 'Одобрить 3 AS6 действия', note: 'Требует вашего решения', value: '+$2,800', icon: '🛡️' },
  { label: 'Отправить Follow-up 12 лидам', note: 'Высокий шанс ответа', value: '+$1,900', icon: '💌' },
]

const recommendations = [
  { title: 'Фокус: Высокий приоритет', text: 'Обработайте лидов из сектора SaaS сегодня. Потенциал: $56,400', action: 'Открыть лиды', href: '/crm?filter=priority', tone: 'violet', icon: '👥' },
  { title: 'Возможность роста', text: 'Увеличьте конверсию на этапе презентации. Потенциал: +$3,200', action: 'Открыть AS6 аналитику', href: '/ai-revenue-intelligence', tone: 'cyan', icon: '📊' },
  { title: 'Оптимизация', text: 'AS6 Сотрудники могут обработать больше лидов. Потенциал: +$2,800', action: 'Настроить AS6', href: '/ai-workforce-center', tone: 'orange', icon: '⚡' },
...
const executiveModules = [
  { title: 'Revenue Brain', text: 'Выручка, воронка, прогноз и возможности роста', href: '/ai-revenue-intelligence', icon: '📈', tone: 'cyan' },
  { title: 'AS6 Workforce', text: 'AS6 Сотрудники, загрузка, эффективность и задачи', href: '/ai-workforce-center', icon: <img src={as6Robot} alt="AS6 Workforce" className="executive-module-robot-icon" />, tone: 'violet' },
  { title: 'Strategic Planning', text: 'Цели, планы, недельные и месячные приоритеты', href: '/ai-strategic-planning', icon: '🧭', tone: 'green' },
  { title: 'Центр исполнения AS6', text: 'Исполнение, операции, действия и контроль результата', href: '/ai-execution-center', icon: '⚡', tone: 'orange' },
  { title: 'Approval Queue', text: 'Очередь решений, approvals и human-in-the-loop контроль', href: '/ai-approval-center', icon: '🛡️', tone: 'pink' },
  { title: 'Состояние системы', text: 'AS6, backend, диагностика, стабильность и readiness', href: '/ai-system-health-center', icon: '🟢', tone: 'blue' },
]
...
const quickActionLinks = {
  ...
  "AS6 аналитика": "/ai-revenue-intelligence",
  "Отчёты": "/dashboard/revenue",
  "Настроить AS6": "/ai-enterprise-coordination",
}

const quickActions = ['Создать лид', 'Создать сделку', 'Добавить задачу', 'Approval Queue', 'AS6 аналитика', 'Отчёты', 'Настроить AS6']
...
const [apiState, setApiState] = useState({})
  ...
  const executiveBrief = useMemo(() => {
    ...
    return {
      title: brief.title || 'AS6 Генеральный директор',
      ...
      focus: brief.focus || apiState.focus?.focus || 'Сфокусироваться на выручке, одобрениях AS6-действий и рисках в pipeline.',
      ...
      opportunity: brief.opportunity || brief.opportunities?.[0] || 'Ускорить обработку лидов через AS6 Workforce и follow-up automation.',
    }
  }, [apiState])
  ...
  const systemStatus = useMemo(() => ([
    ...
    { label: 'AS6 Workforce', value: pickMetric(apiState.kpi?.kpis?.workforceStatus, 'READY') },
  ...
  const actionQueue = useMemo(() => {
    ...
    if (Array.isArray(list) && list.length) return list.slice(0, 4).map((item, index) => ({
      title: item.title || item.name || `AS6 действие #${index + 1}`,
      meta: item.status || item.priority || 'Требует решения',
      value: item.impact || item.value || 'Review',
    }))
    return [
      { title: 'Одобрить AS6 follow-up', meta: 'Approval Queue · высокий приоритет', value: '+$2,800' },
      { title: 'Проверить риск сделки', meta: 'Pipeline · требуется внимание', value: 'Risk' },
      { title: 'Запустить AS6 outreach', meta: 'Growth · готово к запуску', value: '+12 лидов' },
    ]
  }, [apiState])
  ...
  const inboxItems = useMemo(() => {
    ...
    if (Array.isArray(list) && list.length) return list.slice(0, 4).map((item, index) => ({
      ...
      meta: item.from || item.source || item.time || 'AS6 Inbox',
    }))
    return [
      { title: 'Новые лиды ожидают квалификации', meta: 'AS6 Inbox · today' },
      { title: '3 действия ожидают approval', meta: 'Approval Queue' },
  ...
  const businessSummary = useMemo(() => ([
    ...
    { label: 'AS6 Workforce', value: pickMetric(apiState.kpi?.kpis?.workforceUtilization, 'READY'), note: 'Исполнение и автоматизация' },
  ...
  const executiveStream = useMemo(() => {
    const items = [
      ...
      { type: 'Inbox', title: 'AS6 Inbox', text: `${inboxItems.length} входящих executive-сигнала.`, status: inboxItems.length ? 'REVIEW' : 'CLEAR' },
  ...
  function handleFirstActionClick(action, destination) {
    trackProductEvent({
      ...
      screen: 'as6_one',
      source: 'as6_one_command_center_orientation',
      version: 'V222.12',
      metadata: {
        action,
        destination,
  ...
  return (
    <>
    <main className="command-center-page" data-command-center-visual="premium-as6" data-as6-diagnostic-page="as6-one-command-center-premium" data-production-marker="AS6OnePage as6-one as6-command-center-visual-foundation as6-one-exact-command-center-clone as6-one-pixel-perfect-command-center as6-command-center-rebrand as6-command-center-structure-parity as6-revenue-flow as6-general-director as6-core"><section className="command-hero" data-as6-diagnostic-hero="executive-command-center">
        <div>
          <h1>AS6 ONE, <span>Владимир!</span> 👋</h1>
          <p>Первая цифровая форма жизни для бизнеса. Управляйте, анализируйте и масштабируйте компанию.</p>
          <div className="command-live-status"><a className={apiLoading ? 'loading' : 'green'} href="/ai-system-health-center">{apiLoading ? 'Синхронизация...' : 'AS6 работает'}</a><a href="/ai-system-health-center">{apiErrors.length ? `API требует авторизацию: ${apiErrors.length}` : 'API подключён'}</a></div>
        </div>
        <div className="command-top-actions" aria-label="Command Center tools">
          ...
          <a href="/followups" className="top-icon" data-badge="12" aria-label="Уведомления"><BellIcon /></a><a href="/priority-inbox" className="top-icon" data-badge="3" aria-label="AS6 коммуникации"><ChatIcon /></a><a href="/ai-system-health-center" className="top-icon" aria-label="Помощь"><HelpIcon /></a><a href="/profile" className="command-avatar top-icon-profile" aria-label="Профиль">A</a>
        </div>
      </section>
      ...
      <section className="quick-actions quick-actions-primary" data-quick-actions data-as6-primary-actions="above-fold"><h2>Центр действий AS6</h2><div>{quickActions.map((action, index) => <a href={quickActionLinks[action] || "#"} role="button" key={action} aria-label={action}><span>{["👥", "$", "🧾", "🛡️", "〽️", "📊", <img src={as6Robot} alt="AS6" className="quick-action-robot-icon" />][index]}</span>{action}</a>)}</div></section>
      ...
      <section className="command-main-grid">
        <div className="command-core">
          <section className="command-top-grid">
            <article className="command-card daily-goal">
              <div className="command-card-head"><h2>Сегодня AS6 рекомендует</h2><span>68%</span></div>
              <p>Закрыть 4 сделки · Ответить 12 клиентам · Перезапустить 3 зависшие сделки · Назначить встречу · Прогноз роста выручки +487 000 ₽</p>
          ...
          <section className="command-second-grid">
            <article className="command-card employee-performance">
              <div className="command-card-head"><h2>Производительность AS6 Сотрудников</h2><select className="command-filter-select" value={workforceRange} onChange={(event) => setWorkforceRange(event.target.value)} aria-label="AS6 workforce period"><option value="today">Сегодня</option><option value="week">Неделя</option><option value="month">Месяц</option></select></div>
        ...
        <aside className="command-right-rail" data-right-action-rail data-copilot-rail data-as6-diagnostic-copilot="right-action-rail">
          <article className="command-card copilot-hero" data-copilot-hero>
            <div className="copilot-top"><h2>AS6 Помощник</h2><span>AS6</span></div>
            <div className="copilot-inline-layout">
              <img className="as6-copilot-inline-logo" src={as6CopilotLogo} alt="AS6 Copilot" />
              <a className="copilot-action-link" href="/ai-executive-brain">Спросить AS6 Помощника</a>
            </div>
          </article>
          ...
          </article><article className="command-card event-card"><div className="command-card-head"><h2>AS6 Timeline</h2><a href="/dashboard">Все</a></div>{events.map((event) => <div className="event-row" key={event.text}><b>{event.icon}</b><span>{event.text}</span><time>{event.time}</time></div>)}</article>
          <article className="command-card next-action-card"><h2>Лучшее следующее действие</h2>{nextActions.map((action) => <div className="next-action" key={action.label}><b>{action.icon}</b><span>{action.label}<small>{action.note}</small></span><strong>{action.value}</strong></div>)}<div className="expected-effect"><span>Ожидаемый эффект</span><strong>+$8,900</strong></div></article>
        </aside>
      </section>

      <section className="command-recommendations" data-ai-recommendations>
        <h2>Рекомендации AS6</h2><div>{recommendations.map((card) => <a className={`recommendation ${card.tone}`} href={card.href} key={card.title} aria-label={card.action}><b>{card.icon}</b><strong>{card.title}</strong><p>{card.text}</p><span className="recommendation-action">{card.action} →</span></a>)}</div>
      </section>
    </main>
    </>
  )
