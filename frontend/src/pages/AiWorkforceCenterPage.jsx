...
import { fetchAiWorkforceActivityStream, fetchAiWorkforceAgents, fetchAiWorkforceAssignments, fetchAiWorkforceEvents, fetchAiWorkforceExecutionPlans, fetchAiWorkforceMetrics, fetchAiWorkforceRealtimeMetrics, fetchAiWorkforceTasks } from '../services/api'
...
export default function AiWorkforceCenterPage() {
  const [state, setState] = useState({ agents: [], tasks: [], assignments: [], plans: [], metrics: {}, events: [], activity: [], realtime: {}, bottlenecks: [], escalations: [], collaboration: {} })

  useEffect(() => {
    Promise.all([fetchAiWorkforceAgents(), fetchAiWorkforceTasks(), fetchAiWorkforceAssignments(), fetchAiWorkforceExecutionPlans(), fetchAiWorkforceMetrics(), fetchAiWorkforceEvents(), fetchAiWorkforceActivityStream(), fetchAiWorkforceRealtimeMetrics()])
      .then(([agents, tasks, assignments, plans, metrics, events, activity, realtime]) => setState({ agents: agents.items || [], tasks: tasks.items || [], assignments: assignments.items || [], plans: plans.items || [], metrics: metrics.metrics || {}, events: events.items || [], activity: activity.items || [], realtime: realtime.metrics || {}, bottlenecks: realtime.bottlenecks || [], escalations: realtime.escalations || [], collaboration: realtime.collaboration || {} }))
