...
- AEC_STAGING_PUBLIC_EDGE_MUST_PRESERVE_DATA_NETWORK_ISOLATION
- AEC_STAGING_ACCESS_MUST_NOT_CONSUME_APPLICATION_BEARER_AUTH
- AEC_STAGING_PUBLIC_RESPONSES_MUST_BE_NOINDEX_AND_NO_STORE
- AEC_STAGING_ACCESS_CREDENTIALS_MUST_REMAIN_ROOT_ONLY_AND_UNTRACKED
- AEC_STAGING_DNS_AND_TLS_ACTIVATION_MUST_FAIL_CLOSED
...
| AUTONOMOUS_CONTROLLER_CLUSTER_CONTRACT_DRIFT | AEC_REQUIRE_AUTONOMOUS_CONTROLLER_CLUSTER_CONTRACT | autonomous-governance-controllers-must-use-shared-fast-contract | ENABLED |
...
| AS6_STAGING_PRICING_PRODUCTION_CHECKOUT_COLLISION | AEC_REQUIRE_STAGING_PAYMENT_SIMULATION_POLICY | staging-plan-test-must-not-use-production-provider-flow | ENABLED |
| AS6_STAGING_REAL_PAYMENT_MUTATION_RISK | AEC_FORBID_STAGING_PAYMENT_PROVIDER_MUTATION | staging-simulation-must-never-contact-payment-provider | ENABLED |
