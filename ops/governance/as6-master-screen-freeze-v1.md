...
The previously recorded Master Screen revision was superseded by the final product-owner-approved Screen 1 attachment revision `(7)`. The final revision has no visible enclosing rectangular frame around the bottom AS6 communication line while preserving its placeholder, microphone, soft internal light, lower light line, geometry, and Living Sky integration.
...
9. The repository binary must match the registered path, dimensions, mode, and checksum exactly.
...
- Reject any repository artifact whose checksum, dimensions, or color mode differs from the canonical record.
