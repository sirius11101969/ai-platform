...
const url = process.env.AS6_URL || 'https://www.as6.ru/command-center?diag=v22239'
const outJson = process.env.AS6_OUT_JSON || '/work/production-dom-geometry.npx.json'
const outPng = process.env.AS6_OUT_PNG || '/work/production-screenshot.npx.png'
...
;(async () => {
  ...
  fs.writeFileSync('/work/playwright-npx-error.txt', String(error && error.stack ? error.stack : error))
  process.exit(1)
})
