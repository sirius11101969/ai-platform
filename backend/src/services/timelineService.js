...
async function listLeadTimeline(userId, workspaceId, leadId) {
  ...
  const result = await pool.query(
    `SELECT * FROM (
       ...
       UNION ALL
       SELECT s.id::text || ':forecast', s.workspace_id, s.lead_id, NULL::uuid AS user_id, 'ai_forecast_updated' AS event_type, 'AI forecast updated' AS title, COALESCE(NULLIF(s.ai_reasoning, ''), NULLIF(s.recommended_next_step, ''), 'AI forecast revenue and close probability refreshed') AS body, 'ai' AS source, jsonb_build_object('probabilityToClose', s.probability_to_close, 'engagementScore', s.engagement_score, 'expectedRevenue', s.expected_revenue, 'forecastCategory', s.forecast_category, 'riskLevel', s.risk_level, 'recommendedNextStep', s.recommended_next_step) AS metadata, s.generated_at AS created_at FROM lead_ai_scores s WHERE s.workspace_id = $1 AND s.lead_id = $2
       UNION ALL
       SELECT s.id::text || ':risk', s.workspace_id, s.lead_id, NULL::uuid AS user_id, 'ai_risk_detected' AS event_type, 'AI risk detected' AS title, COALESCE(NULLIF(s.ai_reasoning, ''), 'AI detected elevated deal risk') AS body, 'ai' AS source, jsonb_build_object('probabilityToClose', s.probability_to_close, 'expectedRevenue', s.expected_revenue, 'forecastCategory', s.forecast_category, 'riskLevel', s.risk_level, 'riskSignals', s.risk_signals) AS metadata, s.generated_at AS created_at FROM lead_ai_scores s WHERE s.workspace_id = $1 AND s.lead_id = $2 AND (s.risk_level = 'high' OR s.forecast_category IN ('at_risk', 'lost_risk'))
       UNION ALL
