...
function parseManagerCount(text) {
  const source = String(text || '').toLowerCase()
  const match = source.match(/(\d{1,4})\s*(?:\+)?\s*(?:manager|managers|sales\s*reps?|менеджер|менеджеров|продавц|сотрудник)/i)
  return match ? Number(match[1]) : 0
}

function estimateDealValue(lead, text) {
  const explicitValue = Number(lead.value || 0)
  if (explicitValue > 0) return explicitValue
  const managerCount = parseManagerCount(`${lead.company || ''} ${text || ''}`)
  if (managerCount >= 60) return 240000
  if (managerCount >= 20) return 150000
  return 90000
}

function calculateProbabilityToClose(score, temperature, explicitProbability) {
  const ranges = {
    hot: [75, 95],
    warm: [45, 70],
    cold: [10, 40],
  }
  const [min, max] = ranges[temperature] || ranges.cold
  const base = Number.isFinite(Number(explicitProbability)) ? Number(explicitProbability) : score
  return clamp(base, min, max)
}

function calculateEngagementScore({ normalizedScore, lead, telegramMessages, emails, activity, text, signals }) {
  let engagement = Math.round(normalizedScore * 0.45)
  if (lead.telegram || telegramMessages.length) engagement += 18
  if (lead.email) engagement += 10
  if (signals.hasBusinessEmail) engagement += 6
  if (lead.company) engagement += 12
  if (String(text || '').trim().length >= 120) engagement += 12
  else if (String(text || '').trim().length >= 40) engagement += 6
  if (lead.source || lead.source === 'landing' || lead.source === 'website') engagement += 5
  engagement += Math.min(10, telegramMessages.filter((message) => message.role === 'user').length * 3)
  engagement += Math.min(8, emails.filter((email) => email.openedAt).length * 4 + emails.filter((email) => email.sentAt).length)
  engagement += Math.min(6, activity.length)
  if (signals.inboundAfterOutbound) engagement += 10
  engagement -= (signals.riskSignals || []).length * 8
  if (signals.emptyMessage) engagement -= 15
  if (signals.spamLike || signals.hasDisposableEmail) engagement -= 20
  return clamp(engagement)
}

function categorizeForecast({ lead, probabilityToClose, riskLevel, confidence, riskSignals }) {
  const stage = lead.stage || lead.status
  if (stage === 'lost') return 'lost_risk'
  if (riskLevel === 'high' || (riskSignals || []).some((signal) => ['proposal_ignored', 'no_reply_7d', 'high_value_stalled'].includes(signal))) return 'at_risk'
  if (['proposal', 'booked'].includes(stage) && confidence >= 75 && probabilityToClose >= 75) return 'committed'
  if (probabilityToClose >= 80) return 'likely'
  if (probabilityToClose >= 60) return 'possible'
  return 'possible'
}
...
function buildLeadSignals(context) {
  ...
  const positiveIntent = /(готов|интерес|оплат|счет|счёт|договор|встреч|демо|кп|предлож|обсудить|выбирает|внедр)/i.test(text)
  ...
  return { lead, notes, telegramMessages, emails, activity, text, lastTouch, lastOutboundAt, lastInboundAt, hoursInactive: hoursSince(lastTouch || lead.updatedAt || lead.createdAt), hoursSinceOutbound: lastOutboundAt ? hoursSince(lastOutboundAt) : 0, hasOutboundTouch: Boolean(lastOutboundAt), inboundAfterOutbound, positiveIntent, negativeIntent, keywordHits, hasBusinessEmail, hasDisposableEmail, isLongMessage, emptyMessage, spamLike, objections, proposalSent, meetingMentioned, followupCount }
}

function scoreLeadContext(context, aiOutput = {}) {
  const signals = buildLeadSignals(context)
  const { lead, telegramMessages, emails, activity, text, hoursInactive, positiveIntent, negativeIntent, keywordHits, hasBusinessEmail, hasDisposableEmail, isLongMessage, emptyMessage, spamLike, objections } = signals
  ...
  const riskSignals = []
  if (signals.hasOutboundTouch && signals.hoursSinceOutbound >= 24 && !signals.inboundAfterOutbound) riskSignals.push(signals.hoursSinceOutbound >= 168 ? 'no_reply_7d' : signals.hoursSinceOutbound >= 72 ? 'no_reply_3d' : 'no_reply_24h')
  if (signals.hasOutboundTouch && signals.proposalSent && signals.hoursSinceOutbound >= 24 && !signals.inboundAfterOutbound) riskSignals.push('proposal_ignored')
  ...
  const preliminaryTemperature = normalizedScore >= 75 ? 'hot' : normalizedScore >= 45 ? 'warm' : 'cold'
  const dealProbability = calculateProbabilityToClose(normalizedScore, preliminaryTemperature, aiOutput.conversionProbability)
  ...
  const engagementScore = calculateEngagementScore({ normalizedScore, lead, telegramMessages, emails, activity, text, signals })
  const engagementLevel = engagementScore >= 75 ? 'hot' : engagementScore >= 45 ? 'warm' : 'cold'
  const isActiveHotLead = engagementLevel === 'hot' && dealProbability >= 75 && hoursInactive <= 24 && !riskSignals.some((signal) => ['negative_signals', 'proposal_ignored', 'no_reply_3d', 'no_reply_7d'].includes(signal))
  const riskLevel = isActiveHotLead ? 'low' : riskSignals.includes('no_reply_7d') || riskSignals.includes('proposal_ignored') || riskSignals.includes('negative_signals') || hoursInactive >= 168 ? 'high' : riskSignals.length || hoursInactive >= 48 ? 'medium' : 'low'
  const dealValue = estimateDealValue(lead, text)
  ...
  const forecastCategory = riskLevel === 'medium' ? 'at_risk' : categorizeForecast({ lead, probabilityToClose: dealProbability, riskLevel, confidence, riskSignals })
  const expectedRevenue = Math.round(dealValue * (dealProbability / 100))

  return { score: normalizedScore, temperature: engagementLevel, budgetProbability, dealProbability, probabilityToClose: dealProbability, urgencyLevel, engagementLevel, engagementScore, riskLevel, forecastCategory, expectedRevenue, dealValue, riskSignals, aiSummary, aiReasoning: aiSummary, nextBestAction, nextBestActionCode: actionCode, idealContactTiming, objectionsDetected: objections, recommendedCta, recommendedChannel, confidence, riskAlert }
}
