...
function hasMeetingConfirmation(lead = {}) {
  ...
  return (lead.queueItems || []).some((item) => item.action_type === 'telegram_meeting_confirmation_draft' && ['pending_approval', 'approved', 'completed', 'executed'].includes(item.status))
}
...
function getCustomerText(actionType) {
  const texts = {
    ...
    telegram_meeting_confirmation_draft: 'Здравствуйте! Подтверждаю demo-созвон. Если время нужно изменить — просто напишите, я подстроюсь.',
...
function selectNextBestAction(lead, now = new Date()) {
  ...
  return {
    actionType,
    title: buildTitle(actionType, lead),
    nextBestAction: actionType,
    reason: buildReason(actionType, lead, now),
    customerText: assertCustomerSafeText(getCustomerText(actionType), { leadId: lead.id, actionType, source: SOURCE }),
    internalContext: {
      stage,
...
function confidenceFor(actionType, lead, staleHours) {
  ...
  if (actionType === 'telegram_meeting_confirmation_draft') return 88
    ...
    `SELECT l.id, l.user_id, l.workspace_id, l.name, l.company, l.status, l.stage, l.source, l.email, l.telegram,
            ...
            COALESCE((SELECT json_agg(json_build_object('id', q.id, 'action_type', q.action_type, 'status', q.status, 'payload', q.payload) ORDER BY q.created_at DESC)
                       ...
                       WHERE q.workspace_id = l.workspace_id AND q.lead_id = l.id
                         AND q.action_type IN ('telegram_meeting_confirmation_draft')), '[]'::json) AS queue_items
       FROM crm_leads l
...
async function hasDuplicateAction(client, { workspaceId, leadId, actionType }) {
  const result = await client.query(
    `SELECT id FROM ai_worker_queue
      ...
      LIMIT 1`,
    [workspaceId, leadId, actionType, ACTIVE_QUEUE_STATUSES, SOURCE]
  )
...
async function createNextBestAction(client, { userId, workspaceId, workerId, runId, lead, action }) {
  const payload = {
    ...
    customerText: assertCustomerSafeText(action.customerText, { leadId: lead.id, actionType: action.actionType, source: SOURCE, phase: 'before_insert' }),
...
async function runNextBestActionEngine({ userId, workspaceId, worker, workerId }) {
  ...
  try {
    ...
    for (const lead of leads) {
      ...
      const duplicate = await hasDuplicateAction(client, { workspaceId, leadId: lead.id, actionType: action.actionType })
      if (duplicate) {
        console.info('[next-best-action] duplicate skipped', { workspaceId, leadId: lead.id, actionType: action.actionType, duplicateId: duplicate.id })
        duplicates.push({ leadId: lead.id, actionType: action.actionType, duplicateId: duplicate.id })
        continue
      }
