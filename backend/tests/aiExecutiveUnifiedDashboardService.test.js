...
function createClient(missingTables = []) {
  return {
    async query(sql) {
      ...
      if (sql.includes('ai_strategic_plans')) return { rows: [{ plan_payload: { horizon: 'q4' } }], rowCount: 1 }
...
async function testMissingCoordinationTable() {
  const overview = await getOverview({ workspaceId: 'w1', client: createClient(['ai_enterprise_coordination_runs']) })
...
async function testMissingSimulationTable() {
  const overview = await getOverview({ workspaceId: 'w1', client: createClient(['ai_company_simulation_risks']) })
