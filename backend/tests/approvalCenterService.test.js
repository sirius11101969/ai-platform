...
function install() {
  ...
  const state = { item: { id: 'a1', workspace_id: 'w1', approval_status: 'pending_approval', recommendation_type: 'next_best_action', recommendation_payload: { suggestedAction: 'Send follow-up' }, confidence_score: 86, urgency: 'high', created_at: new Date().toISOString(), updated_at: new Date().toISOString() }, audits: 0 }
  const client = { query: async (sql, params=[]) => { if (['BEGIN','COMMIT','ROLLBACK'].includes(sql)) return { rows: [] }; if (sql.startsWith('INSERT INTO ai_approval_queue')) return { rows: [state.item] }; if (sql.startsWith('SELECT q.*, l.name')) return { rows: [state.item] }; if (sql.startsWith('SELECT * FROM ai_approval_queue')) return { rows: [state.item] }; if (sql.startsWith('INSERT INTO ai_approval_decisions')) return { rows: [{ id: 'd1' }] }; if (sql.startsWith('UPDATE ai_approval_queue')) { state.item = { ...state.item, approval_status: params[2], updated_at: new Date().toISOString() }; return { rows: [state.item] } } if (sql.startsWith('INSERT INTO ai_approval_audit_log')) { state.audits += 1; return { rows: [{ id: 'log1' }] } } throw new Error(`Unexpected query ${sql}`) }, release(){} }
...
async function main() {
  ...
  const list = await service.listQueue('w1')
